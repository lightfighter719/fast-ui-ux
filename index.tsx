...
- **NO WEB3/METAMASK**: Do not include any web3, ethereum, or MetaMask related code.
...
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions (e.g. ["Tactile Risograph Press", "Kinetic Silhouette Balance", "Primary Pigment Gridwork"]).
        ...
        const generateArtifact = async (artifact: Artifact, styleInstruction: string, retries = 1) => {
            try {
                const prompt = `
You are Fast UI. Create a stunning, high-fidelity UI component for: "${trimmedInput}".
...
6. **NO WEB3/METAMASK**: Do not include any web3, ethereum, or MetaMask related code or detection logic. This is a pure UI/UX prototype.
...
Return ONLY RAW HTML. No markdown fences.
          `.trim();
                ...
                // Retry logic for 503 or parsing errors
                if (retries > 0 && (e.message?.includes('503') || e.message?.includes('parsing'))) {
                    console.log(`Retrying artifact ${artifact.id} due to error: ${e.message}`);
                    await new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 500));
                    return generateArtifact(artifact, styleInstruction, retries - 1);
                }
        ...
        // Stagger requests to prevent 503 errors from concurrency limits
        const promises = placeholderArtifacts.map(async (art, i) => {
             await new Promise(resolve => setTimeout(resolve, i * 300)); // 300ms stagger
             return generateArtifact(art, generatedStyles[i]);
        });
  ...
  return (
    <>
        <a href="https://fast.jessejesse.com" target="_blank" rel="noreferrer" className="app-logo">
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="40" height="40" rx="8" fill="url(#paint0_linear)"/>
                <path d="M22 10L12 22H18L16 30L26 18H20L22 10Z" fill="white"/>
                <defs>
        ...
        <a href="https://x.com/lightfighter719" target="_blank" rel="noreferrer" className={`creator-credit ${hasStarted ? 'hide-on-mobile' : ''}`}>
            @lightfighter719
        </a>
        ...
        <div className="immersive-app">
            <Feed sessions={sessions} onSelectSession={(idx) => {
            ...
            <div className={`stage-container ${focusedArtifactIndex !== null ? 'mode-focus' : 'mode-split'}`}>
                 <div className={`empty-state ${hasStarted ? 'fade-out' : ''}`}>
                     <div className="empty-content">
                         <h1>Fast UI</h1>
                         <p>INSTANT UI FOR ANY PROJECT</p>
            ...
            <a href="https://fast.jessejesse.com" target="_blank" rel="noreferrer" className="subtle-footer">
                fast.jessejesse.com
            </a>
        </div>
    </>
...
if (rootElement) {
  ...
  if (!win._root) {
    win._root = ReactDOM.createRoot(rootElement);
  }
  win._root.render(<React.StrictMode><App /></React.StrictMode>);
}