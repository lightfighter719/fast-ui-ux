...
export default function DottedGlowBackground({
  ...
  useEffect(() => {
    ...
    let dots: { x: number; y: number; phase: number; speed: number; isYellow: boolean }[] = [];
      ...
      for (let i = -1; i < cols; i++) {
        for (let j = -1; j < rows; j++) {
          ...
          dots.push({
            ...
            isYellow: Math.random() < 0.12, // ~12% chance for a yellow star
          });
        }
      }
    };
    ...
    const draw = (now: number) => {
      ...
      const yellowColor = "rgba(255, 230, 100, 0.15)";
      const yellowGlowColor = "rgba(255, 215, 0, 0.9)";
      ...
      dots.forEach((d) => {
        ...
        const dotColor = d.isYellow ? yellowColor : color;
        const dotGlow = d.isYellow ? yellowGlowColor : glowColor;
